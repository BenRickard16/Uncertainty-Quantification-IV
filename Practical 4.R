...
## 3. Improving Emulator Efficiency

# Want to improve computational speed of calculating covariance matrices and its inverse by distance function
xD <- matrix(c(1, 1,
               2, 2,
...
# Or can use pdist() from pdist library
...
## 4. Testing the New Emulator Function

# Function we are emulating
f_2 <- function(x) 2*sin( 2*pi*(x[,2]/2 + 0 + (x[,1]-1)^2+(x[,2]-1)^2))

# Define run locations
set.seed(1)
xD_w1 <- lhd_maximin(n=14)     # function from practical 3
...
# Use function system.time() to check how much faster the new code is
...
emul_fill_cont(cont_mat=E_D_fx_mat,cont_levs=NULL,xD=xD,x_grid=x_grid,             # function from previous practical
...
## 5. Different Correlation Lengths
...
# Ellipses appear due to the differing correlation lengths



## 6. Optimisation of an Expensive Function

# Example of Optimisation using an Implausibility Strategy
...
# Perform a second wave in the non-implausible regions
...
for(k in 0:8){                          # k=0: wave 1, k>0 add k wave 2 runs sequentially
  ...
  if(k==nrow(xD_w2)) emul_fill_cont_V2(cont_mat=Imp_true_mat,cont_levs=imp_levs,cont_levs_lines=3,xD=xD,x_grid=x_grid,
                                       xD_col="purple",plot_xD=FALSE,color.palette=imp_cols,
                                       main="Implausibility I(x) using True f(x)")
}
